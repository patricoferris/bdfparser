module Glyph = struct
  ...
  let name (g : t) : string = g.name
  let bbox (g : t) : int * int * int * int = g.bounding_box
  let dwidth (g : t) : int * int = g.dwidth
...
let glyph_count t = Array.length t.glyphs
...
let innerchar_to_glyph (ic : Types.char_property_val list) : Glyph.t =
  List.fold_left
    (fun (acc : Glyph.t) (item : Types.char_property_val) : Glyph.t ->
      match item with
          ...
          List.iteri
            (fun r v ->
              for c = 0 to bytes_per_line - 1 do
                Bytes.set bitmap
                  ((r * bytes_per_line) + c)
                  (char_of_int (Int.shift_right v (8 * c) land 0xFF))
              done)
            d;
          { acc with bitmap })
    Glyph.default ic
        ...
        (let fnt =
           List.fold_left
             (fun (acc : t) (item : Types.header) : t ->
               ...
               | `Properties p ->
                   { acc with properties = List.concat [ acc.properties; p ] }
               | `Char c ->
                   let g = innerchar_to_glyph c in
                   {
                     acc with
                     glyphs = Array.of_list (g :: Array.to_list acc.glyphs);
                   }
               | `Noop -> acc)
             default_t ast
         in
         ...
         let map =
           Array.mapi (fun i g -> (Uchar.of_int g.Glyph.encoding, i)) fnt.glyphs
         in
         { fnt with map = Array.to_list map })

let name t = t.name
let bdf_version t = t.version
let version t = t.content_version
