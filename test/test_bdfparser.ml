...
ENDFONT|} in
  let lexbuf = Lexing.from_string prose in
  let ast = Bdf.Parser.prog Bdf.Lexer.read lexbuf in
  let expected = [ `Version 2.1; `Size (1, 2, 3); `Noop ] in
  match ast with
  | None -> assert_failure "Got nothing"
  | Some l -> assert_equal l expected
...
ENDFONT|} in
  let lexbuf = Lexing.from_string prose in
  let ast = Bdf.Parser.prog Bdf.Lexer.read lexbuf in
  let expected = [ `Version 2.1; `BoundingBox (1, 2, 3, 4); `Noop ] in
  match ast with
  | None -> assert_failure "Got nothing"
  | Some l -> assert_equal l expected
...
ENDFONT|} in
  let lexbuf = Lexing.from_string prose in
  let ast = Bdf.Parser.prog Bdf.Lexer.read lexbuf in
  let expected = [ `Version 2.1; `ContentVersion 42; `Noop ] in
  match ast with
  | None -> assert_failure "Got nothing"
  | Some l -> assert_equal l expected
...
ENDFONT|} in
  let lexbuf = Lexing.from_string prose in
  let ast = Bdf.Parser.prog Bdf.Lexer.read lexbuf in
  let expected = [ `Version 2.1; `MetricSet 1; `Noop ] in
  match ast with
  | None -> assert_failure "Got nothing"
  | Some l -> assert_equal l expected
...
ENDFONT|} in
  let lexbuf = Lexing.from_string prose in
  let ast = Bdf.Parser.prog Bdf.Lexer.read lexbuf in
  let expected = [ `Version 2.1; `Chars 0; `Noop ] in
  match ast with
  | None -> assert_failure "Got nothing"
  | Some l -> assert_equal l expected
...
ENDFONT|} in
  ...
  let ast = Bdf.Parser.prog Bdf.Lexer.read lexbuf in
  let expected =
    [ `Version 2.1; `Chars 1; `Char [ `CharName "char0000" ]; `Noop ]
  in
  match ast with
  | None -> assert_failure "Got nothing"
  | Some l -> assert_equal l expected
...
ENDFONT|}
  ...
  let expected =
    [
      `Version 2.1;
      `Chars 1;
      `Char [ `CharName "char0000"; `Encoding 42 ];
      `Noop;
    ]
  in
  match ast with
...
ENDFONT|}
  ...
  let expected =
    [
      `Version 2.1;
      `Chars 1;
      `Char [ `CharName "char0000"; `SWidth (1, 2); `DWidth (3, 4) ];
      `Noop;
    ]
  in
  match ast with
